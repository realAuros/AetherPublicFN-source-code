...
bool ImGui::Tab(const char* label, ImTextureID icon, bool active)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddImage(icon, rect.Min + ImVec2(17, 12), rect.Min + ImVec2(33, 28), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text));
    window->DrawList->AddText(fonts::font2,18, rect.Min + ImVec2(40, 10), ImColor(it_anim->second.text), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
bool ImGui::TextLine(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddText(fonts::font2, 18, rect.Min + ImVec2(10, 5), ImColor(140,140,140), label);
    window->DrawList->AddRectFilled(rect.Min + ImVec2(CalcTextSize(label).x,13), rect.Min + ImVec2(130,15), ImColor(15, 15, 15), 5);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
struct checkbox_state {
    ImVec4 framecol, checkmark;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.checkmark = ImLerp(it_anim->second.checkmark, *v ? ImColor(1,1,1) : hovered ? ImColor(71, 71, 73) : ImColor(0,0,0,0), g.IO.DeltaTime * 4.f);
    ...
    RenderCheckMark(window->DrawList, frame_bb.GetCenter() - ImVec2((square_sz / 2 + 1) / 2, (square_sz / 2 + 1) / 2), GetColorU32(it_anim->second.checkmark), square_sz / 2 + 1);
...
bool ImGui::Keybind(const char* label, int* key) {
    ...
    const ImGuiID id = window->GetID(label);
    const float size = GetWindowWidth() - (g.Style.WindowPadding.x * 2) - 8;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size , 32));
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size + 4, 0), window->DC.CursorPos + ImVec2(size, 31));
    ImRect key_rect;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(270, 30));
    ...
    key_rect = ImRect(mode_rect.Min - ImVec2(CalcItemWidth(), 0), ImVec2(mode_rect.Min.x - 1, mode_rect.Max.y + 8));
    ...
    if (*key != 0 && g.ActiveId != id) {
        ...
        strcpy_s(buf_display1, "Waiting for key...");
    }
    ...
    if (hovered_key && io.MouseClicked[0])
    {
        ...
        io.MouseClicked[0] = false; // prevent immediate click registration
    }
    else if (io.MouseClicked[0] && !hovered_key && g.ActiveId == id)
    {
        // If user clicks anywhere else, stop listening
        ImGui::ClearActiveID();
    }
    ...
    it_anim->second.mode_color = ImLerp(it_anim->second.mode_color, IsItemActive() ? ImColor(255,255,255,255) : hovered_rect ? ImColor(150, 150, 150, 150) : ImColor(107, 107, 107, 107), g.IO.DeltaTime * 6.f);

    ImGui::PushFont(fonts::Regylar);

    window->DrawList->AddText(rect.Min + ImVec2(0, 3), ImColor(255,255,255,255), label);

  

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.mode_color));

    window->DrawList->AddRectFilled(key_rect.Min + ImVec2(CalcItemWidth() - 9, 2), key_rect.Max - ImVec2(-10, 12), ImColor(24, 25, 27, 255), 2.f);

    RenderTextClipped(key_rect.Min - ImVec2(23, 29), key_rect.Max + ImVec2(CalcItemWidth() + 24, 19), buf_display1, NULL, NULL, ImVec2(0.5f, 0.5f));
...
void ImGui::CustomSpacing(float h)
{
    ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags, bool multi)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImGui::SetCursorPos(GetCursorPos() + ImVec2(6, 28));
    ...
    const ImRect bb(window->DC.CursorPos - ImVec2(8,35), window->DC.CursorPos + ImVec2(GetWindowWidth() - 25, 3));
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? 26 * val : 0.f, g.IO.DeltaTime * 8.f);

    it_anim->second.background = ImLerp(it_anim->second.background, it_anim->second.opened_combo ? ImColor(20,20,20) : hovered ? ImColor(20,20,20) : ImColor(20,20,20), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : hovered ? ImColor(150, 150, 150, 150) : ImColor(107, 107, 107, 107), g.IO.DeltaTime * 6.f);


    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 , bb.Max.y), GetColorU32(it_anim->second.background),4.f, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);

    window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, GetColorU32(it_anim->second.background), 4.f, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);

    RenderFrameBorder(bb.Min, bb.Max, style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    PushFont(fonts::Regylar);
    RenderTextClipped(bb.Min + ImVec2(10,8), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopFont();
    PushFont(fonts::ico_list);
    RenderTextClipped(bb.Min + ImVec2(170,11), ImVec2(value_x2 + 10, bb.Max.y), "K", NULL, NULL);
    PopStyleColor();
    PopFont();
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y - 31));
    PushStyleVar(ImGuiStyleVar_WindowRounding, 4);
    PushStyleColor(ImGuiCol_WindowBg, ImVec4(20/255.f,20/255.f,20/255.f,1.f));
    ...
    if (multi)
    {
        if (IsMouseHoveringRect(GetWindowPos(), GetWindowPos() + GetWindowSize()) && g.IO.MouseClicked[0])
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 23));
    PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 3));
...
void ImGui::MultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ...
    if (ImGui::BeginCombo(label, preview.c_str(), count)) 
    {

        static std::map<ImGuiID, comboA_state> anim;
        auto it_anim = anim.find(g.CurrentWindow->ID);

        if (it_anim == anim.end())
        {
            anim.insert({ g.CurrentWindow->ID, comboA_state() });
            it_anim = anim.find(g.CurrentWindow->ID);
        }
        ...
        SetCursorPos(GetCursorPos() + ImVec2(0, 23));
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y + 30));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 60, label_size.y + 5));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(255,255,255) : hovered ? ImColor(120,120,120) : ImColor(70,70,70), g.IO.DeltaTime * 12.f);
    it_anim->second.hint_slider = ImLerp(it_anim->second.hint_slider, IsItemHovered() ? 1.f : 0.f, g.IO.DeltaTime * 35.f);

    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(0, 11),ImColor(20,20,20,100), 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Min + ImVec2(it_anim->second.slow - 7, 16.5f), ImColor(accentcolor), 100, ImDrawFlags_RoundCornersLeft);
        const int vtx_idx_2 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        window->DrawList->AddCircleFilled(frame_bb.Min + ImVec2(it_anim->second.slow - 1, 12.f), 8, ImColor(255,255,255), 100);
    ...
    PushFont(fonts::Regylar);
    RenderTextClipped(frame_bb.Min - ImVec2(0, 22), frame_bb.Max - ImVec2(0, 22), value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));
    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w - 60, frame_bb.Min.y - 18), label);
    PopFont();
    PopStyleColor();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 bar_pos = window->DC.CursorPos + ImVec2(0, 141);
    float square_sz = GetFrameHeight();
    float bars_width = 210.f; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)) + 3; // Saturation/Value picking box
    float sv_bar_size = 15; // Saturation/Value picking box
    float bar0_pos_x = GetWindowPos().x + 17;
    ...
    // Render SV Square
    draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 6.f, ImDrawFlags_RoundCornersTop);
    draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 6.f, ImDrawFlags_RoundCornersBottom);
    ...
    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x + i * (bars_width / 6), bar_pos.y + 4), ImVec2(bar0_pos_x + (i + 1) * (bars_width / 6), bar_pos.y + sv_bar_size - 5), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i]);
    draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 6.f, 1);

    float bar0_line_x = IM_ROUND(bar_pos.x + (1.f - H) * bars_width);

    bar0_line_x = ImClamp(bar0_line_x, picker_pos.x + 5.f, picker_pos.x + 207.f);

    it_anim->second.hue_bar = ImLerp(it_anim->second.hue_bar, bar0_line_x - bar_pos.x, g.IO.DeltaTime * 20.f);

    draw_list->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 6.9f), 5, hue_color32, 100);
    draw_list->AddCircle(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 6.9f), 6, ImColor(255, 255, 255, 255), 100, 1.5f);
    ...
    draw_list->AddShadowCircle(it_anim->second.circle_move + bar_pos, it_anim->second.circle, ImColor(0.1f, 0.1f, 0.1f, 1.f), 30.f, ImVec2(0, 0), 32);
    draw_list->AddCircleFilled(it_anim->second.circle_move + bar_pos, it_anim->second.circle, user_col32_striped_of_alpha, 32);
    draw_list->AddCircle(it_anim->second.circle_move + bar_pos + ImVec2(0, 1), it_anim->second.circle, IM_COL32(50, 50, 50, 150 * g.Style.Alpha), 32, 1.3f);
    draw_list->AddCircle(it_anim->second.circle_move + bar_pos, it_anim->second.circle, IM_COL32(230, 230, 230, 255 * g.Style.Alpha), 32, 1.3f);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilled(bar1_bb.Min + ImVec2(0, 4), bar1_bb.Max - ImVec2(0, 5), ImColor(0, 0, 0, 255), style.FrameRounding);
        draw_list->AddRectFilledMultiColor(bar1_bb.Min + ImVec2(0, 4), bar1_bb.Max - ImVec2(0, 5), user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK);
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 4), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 6.f, 0);
        float bar1_line_x = IM_ROUND(bar_pos.x + alpha * bars_width);

        bar1_line_x = ImClamp(bar1_line_x, bar_pos.x, picker_pos.x + 202.f);
        it_anim->second.alpha_bar = ImLerp(it_anim->second.alpha_bar, bar1_line_x - bar_pos.x + 5.f, g.IO.DeltaTime * 20.f);
        draw_list->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 7.0f), 6, IM_COL32(255, 255, 255, 255), 100);
    }
    ...
    // draw_list->AddRectFilled(g.CurrentWindow->Pos + ImVec2(158, 14), g.CurrentWindow->Pos + ImVec2(188, 39), ImColor(ref_col_v4), 5.f);
   // draw_list->AddRect(g.CurrentWindow->Pos + ImVec2(158, 14), g.CurrentWindow->Pos + ImVec2(188, 39), ImColor(0.1f, 0.1f, 0.1f, 1.f), 4.f, ImDrawListFlags_None, 1.2f);

   //  draw_list->AddRectFilled(g.CurrentWindow->Pos + ImVec2(198, 14), g.CurrentWindow->Pos + ImVec2(228, 39), ImColor(col[0], col[1], col[2], col[3] * g.Style.Alpha), 5.f);
     //draw_list->AddRect(g.CurrentWindow->Pos + ImVec2(198, 14), g.CurrentWindow->Pos + ImVec2(228, 39), ImColor(0.1f, 0.1f, 0.1f, 1.f), 4.f, ImDrawListFlags_None, 1.2f);

    const int vtx_idx_1 = ImGui::GetForegroundDrawList()->VtxBuffer.Size;
    //GetForegroundDrawList()->AddRect(g.CurrentWindow->Pos + ImVec2(0, 0), g.CurrentWindow->Pos + ImVec2(242, flags & ImGuiColorEditFlags_AlphaBar ? 238 : 220), ImColor(0.1f, 0.1f, 0.1f, 1.f), style.PopupRounding, ImDrawListFlags_None, 1.2f);
    ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetForegroundDrawList(), vtx_idx_1, ImGui::GetForegroundDrawList()->VtxBuffer.Size, g.CurrentWindow->Pos + ImVec2(0, 0), g.CurrentWindow->Pos + ImVec2(0, flags & ImGuiColorEditFlags_AlphaBar ? 190 : 172), ImColor(0.1f, 0.1f, 0.1f, 1.f), ImColor(0.1f, 0.1f, 0.1f, 1.f));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    const ImVec2 label_size = ImGui::CalcTextSize(desc_id, NULL, true);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(default_size + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 50.0f), label_size.y + style.FramePadding.y * 2.0f));
    if (!ItemAdd(total_bb, id))
        return false;
    if (size.x == 0.0f)
        size.x = default_size;
    if (size.y == 0.0f)
        size.y = default_size;
    const ImRect bb(window->DC.CursorPos + ImVec2(190, 0), window->DC.CursorPos + ImVec2(225, label_size.y - 30) + size);
    ...
    else
    {
        ...
        ImGui::PushFont(fonts::ico);

        ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(col_source));

        RenderText(ImVec2(bb_inner.Max.x - 75, bb_inner.Min.y - 5), "U");
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? ImColor(accentcolor) : hovered ? ImColor(71, 71, 73, 255) : ImColor(51, 51, 53, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.mark_color = ImLerp(it_anim->second.mark_color, selected ? ImColor(accentcolor) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    RenderTextClipped(text_min + ImVec2(5, 0), text_max + ImVec2(25, 0), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    RenderCheckMark(GetWindowDrawList(), ImVec2(text_min.x + GetWindowContentRegionMax().x - 23, text_max.y - 14), GetColorU32(it_anim->second.mark_color), 10.f);
    PopStyleColor();
