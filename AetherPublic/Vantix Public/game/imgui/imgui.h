...
namespace ImGui
{
    ...
    IMGUI_API void          CustomSpacing(float h);                                                      // add vertical spacing.
    ...
    IMGUI_API bool          TextLine(const char* label);
    IMGUI_API bool          Keybind(const char* label, int* key);
    IMGUI_API bool          Tab(const char* label, ImTextureID icon, bool active);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags = 0, bool multi = false);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_max_height_in_items = -1);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API void          MultiCombo(const char* label, bool variable[], const char* labels[], int count = 0);
