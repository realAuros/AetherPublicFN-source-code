...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (Status == 0xC0000022 || Status == 0xC000009A) { //STATUS_ACCESS_DENIED and STATUS_INSUFFICIENT_RESOURCES
		std::cout << ("[!] Error 09\n Please make sure to close the game before launching the software") << "\n";
		Sleep(1500);
		BlockInput(0);
		exit(1);
	}
	...
	//Never should occur since eterna checks for "IsRunning" driver before
