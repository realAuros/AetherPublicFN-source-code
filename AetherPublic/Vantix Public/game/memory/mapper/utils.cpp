...
std::wstring eterna_utils::GetFullTempPath() {
...
bool eterna_utils::ReadFileToMemory(const std::wstring& file_path, std::vector<uint8_t>* out_buffer) {
...
bool eterna_utils::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t eterna_utils::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN eterna_utils::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t eterna_utils::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
...
PVOID eterna_utils::FindSection(const char* sectionName, uintptr_t modulePtr, PULONG size) {
