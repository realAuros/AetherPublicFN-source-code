...
std::wstring intel_driver::GetDriverPath() {
	std::wstring temp = eterna_utils::GetFullTempPath();
	if (temp.empty()) {
		return L"";
	}
...
HANDLE intel_driver::Load() {
	...
	if (!eterna_utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		return INVALID_HANDLE_VALUE;
	}
	...
	ntoskrnlAddr = eterna_utils::GetKernelModuleAddress("ntoskrnl.exe");
	if (ntoskrnlAddr == 0) {
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	auto WdFilter = eterna_utils::GetKernelModuleAddress("WdFilter.sys");
	if (!WdFilter) {
		return true;
	}
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	...
	auto result = eterna_utils::FindPattern((uintptr_t)sectionData.get(), dwLen, bMask, szMask);
...
uintptr_t intel_driver::FindSectionAtKernel(HANDLE device_handle, const char* sectionName, uintptr_t modulePtr, PULONG size) {
	...
	ULONG sectionSize = 0;
	uintptr_t section = (uintptr_t)eterna_utils::FindSection(sectionName, (uintptr_t)headers, &sectionSize);
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	uint64_t ci = eterna_utils::GetKernelModuleAddress("ci.dll");
	if (!ci) {
		return false;
	}
